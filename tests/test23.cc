...
/* Test modifying segment owned by other transaction */
...
public:
  ...
  node *new_tail_node(void)
  {
    ...
    rvm_about_to_modify(trans1, tail_node, sizeof(node *), sizeof(node *));
    tail_node->set_next(create_new_node());

    trans2_seg[0] = tail_node->get_next();
    trans2 = rvm_begin_trans(rvm, 1, trans2_seg);

    rvm_about_to_modify(trans1, tail_node->get_next(), 0, sizeof(node));
    rvm_about_to_modify(trans1, tail_node->get_next(), sizeof(node *), sizeof(node *));
